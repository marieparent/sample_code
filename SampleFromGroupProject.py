"""Sample Code from Group Project CS61A at UC Berkeley."""
...
class Bee(Insect):
    """A Bee moves from place to place, following exits and stingimng ants."""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        range_list = [self.place]
        while range_list[-1] != hive:
            range_list.append(range_list[-1].entrance)
        if hasattr(self, 'min_range'):
            range_start = self.min_range
        else:
            range_start = 0
        if hasattr(self, 'max_range'):
            range_end = self.max_range
        else:
            range_end = len(range_list) - 1
        target = None
        while (target == None
            and range_start <= range_end
            and range_list[range_start] != hive):
            target = random_or_none(range_list[range_start].bees)
            range_start += 1
        return target
        # BEGIN Problem 5
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        kamakaze_bee_list = self.place.bees
        if self.armor <= amount:
        	for bee in list(kamakaze_bee_list):
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        ninja_action_list = self.place.bees
        for bee in list(ninja_action_list):
            bee.reduce_armor(self.damage)
        # END Problem 8
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        ...
        if self.can_contain(ant):
            ...
            return 'The argument ant cannot be contained by this ant.'
        # END Problem 11
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 12
        tank_action_list = self.place.bees
        for bee in list(tank_action_list):
            bee.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            if self.places_behind_queen == []:
                self.gen_places_behind_queen()
            i = 0
            while i < len(self.places_behind_queen) - 1:
                if self.places_behind_queen[i].ant and self.places_behind_queen[i].ant not in self.queens_list:
                    self.queens_list.append(self.places_behind_queen[i].ant)
                    self.places_behind_queen[i].ant.damage *= 2
                # Also, if the added ant is a container, add its contained ant
                if self.places_behind_queen[i].ant and self.places_behind_queen[i].ant.container and self.places_behind_queen[i].ant.ant and self.places_behind_queen[i].ant.ant not in self.queens_list: # something wrong on this line
                    self.queens_list.append(self.places_behind_queen[i].ant.ant)
                    self.places_behind_queen[i].ant.ant.damage *= 2
                i += 1

    def gen_places_behind_queen(self):
            self.places_behind_queen = [self.place.exit]
            while self.places_behind_queen[-1].exit != None:
                self.places_behind_queen.append(self.places_behind_queen[-1].exit)
        # END Problem 13
...
def make_slow(action, duration, bee):
    """Return a new action method that calls ACTION every other turn.

    action -- An action method of some Bee
    """
    # BEGIN Problem EC
    def slow_implement(colony):
        if (colony.time % 2 == 0):
            action(colony)
        bee.effect_duration -= 1
        if bee.effect_duration <=0:
            bee.effect_list.pop()
            if not len(bee.effect_list) == 0:
                new_effect_data = bee.effect_list.pop()
                bee.action = new_effect_data[0]
                bee.effect_duration = new_effect_data[1]
                bee.effect_list.append(new_effect_data)
            else:
                bee.action = bee.default_action
    return slow_implement

    # END Problem EC

def make_stun(action, duration, bee):
    """Return a new action method that does nothing.

    action -- An action method of some Bee
    """
    # BEGIN Problem EC
    def stun_implement(colony):
        bee.effect_duration -= 1
        if bee.effect_duration <= 0:
            bee.effect_list.pop()
            if not len(bee.effect_list) == 0:
...
def apply_effect(effect, bee, duration):
    ...
    if not hasattr(bee, 'default_action'):
        bee.default_action = bee.action
    elif bee.default_action == None:
        bee.default_action = bee.action
    bee.action = effect(bee.action, duration, bee)
    if not hasattr(bee, 'effect_list'):
        bee.effect_list = []
    bee.effect_list.append([bee.action, duration, bee])
    # END Problem EC
